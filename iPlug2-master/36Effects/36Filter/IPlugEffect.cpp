...
double getStatR(const double& x) {
  if (x <= 0.0)
    return my_stat[0][0] - 1.1;
  else if (x >= 1.0)
    return my_stat[0][9] - 1.1;
  return my_stat[0][int(x * 10.0)] - 1.1;
}

double getStatL(const double& x) {
  if (x <= 0.0)
    return my_stat[1][0] - 1.1;
  else if (x >= 1.0)
    return my_stat[1][9] - 1.1;
    ...
    */
    {
      ...
      pGraphics->AttachControl(new IVPlotControl(cell(0, 2).Union(cell(0, 2)), { {COLOR_DARK_GRAY , [](double x) { return getSigmoid(x); } } }, 32, "", style), kNoTag, "vcontrols");
      ...
      pGraphics->AttachControl(new IVSlideSwitchControl(cell(0, 5).GetMidVPadded(buttonSize), sendDiff, "Only Diff", style.WithShowValue(false).WithShowLabel(true).WithWidgetFrac(0.5f).WithDrawShadows(false), false), kNoTag, "vcontrols");
