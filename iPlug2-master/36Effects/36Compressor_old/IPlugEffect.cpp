...
double getSigmoid(const double& x) {
  return 2.4 * sigmoid.get(x) - 1.2;
}

double getValueDisplay(const double& x) {
  if (x <= .2) return (x * 5.0) * (_cp0) * 2. - 1.2;
  else if (x < .4) return ((x - .2) * 5.0 * (_cp1 - _cp0) + _cp0) * 2. - 1.2;
  else if (x < .6) return ((x - .4) * 5.0 * (_cp2 - _cp1) + _cp1) * 2. - 1.2;
  else if (x < .8) return ((x - .6) * 5.0 * (_cp3 - _cp2) + _cp2) * 2. - 1.2;
  else return ((x - .8) * 5.0 * (_cp4 - _cp3) + _cp3) * 2. - 1.2;
}

double getStatR(const double& x) {
  return my_stat[0][int(x * 10.0)] - 1.1;
}

double getStatL(const double& x) {
  return my_stat[1][int(x * 10.0)] - 1.1;
}

IPlugEffect::IPlugEffect(const InstanceInfo& info)
: Plugin(info, MakeConfig(kNumParams, kNumPrograms)), UIClosed(true), displayCount(0), isInit(false),
atStartCount(0), bsendDiff(false), breplot(false)
{
  ...
  GetParam(dcR)->InitDouble("DC-R", .999, 0.98, .999, 0.001);
  GetParam(ingain)->InitDouble("Gain In", 1., 0.0, 2.0, 0.01);
  GetParam(limiterType)->InitEnum("Limiter type", 0, 2, "", IParam::kFlagsNone, "", "Mirror", "Trunc");
  GetParam(threshold)->InitDouble("Threshold", 0.3, 0.0, 1.0, 0.01);
  GetParam(maxTresh)->InitDouble("Max Threshold", 1.5, 1.0, 2.0, 0.05);
  ...
  GetParam(cp4)->InitDouble("Half-", 1.2, 0.0, 1.2, 0.01);
  GetParam(outgain)->InitDouble("Gain Out", 1., 0.0, 2.0, 0.01);
  GetParam(steep)->InitDouble("Stiffness", .1, 0.1, 20.0, 0.1);
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    const IVStyle style2{
     ...
     false, // Show value
     {
       ...
       IColor(0, 255, 255, 255), // Foreground
       IColor(0, 255, 255, 255), // Pressed
       COLOR_BLACK, // Frame
    ...
    */
    {
      ...
      // upper left
      pGraphics->AttachControl(new IVKnobControl(cell(0, 0).GetMidVPadded(buttonSize), steep, "Stiffness", style, false), kNoTag, "vcontrols");
      pGraphics->AttachControl(new IVPlotControl(cell(0, 1).Union(cell(0, 1)), { {COLOR_DARK_GRAY , [](double x) { return getSigmoid(x); } } }, 32, "", style), kNoTag, "vcontrols");
      pGraphics->AttachControl(new IVSlideSwitchControl(cell(0, 2).GetMidVPadded(buttonSize), replot, "Replot", style.WithShowValue(false).WithShowLabel(true).WithWidgetFrac(0.5f).WithDrawShadows(false), false), kNoTag, "vcontrols");



      pGraphics->AttachControl(new IVKnobControl(cell(4, 3).GetMidVPadded(buttonSize), mix, "Mix", style, false), kNoTag, "vcontrols");

      //mid
      pGraphics->AttachControl(new IVSlideSwitchControl(cell(1, 1).Union(cell(1, 2)).GetMidVPadded(buttonSize), limiterType, "Limiter", style, true), kNoTag, "vcontrols");
      


      //down left
      pGraphics->AttachControl(new IVKnobControl(cell(2, 0).GetMidVPadded(buttonSize), outgain, "Gain Out", style, false), kNoTag, "vcontrols");
      pGraphics->AttachControl(new IVKnobControl(cell(2, 1).GetMidVPadded(buttonSize), dcR, "DC-R", style, false), kNoTag, "vcontrols");
      pGraphics->AttachControl(new IVKnobControl(cell(2, 2).GetMidVPadded(buttonSize), threshold, "Threshold", style, false), kNoTag, "vcontrols");

      pGraphics->AttachControl(new IVMeterControl<2>(cell(3, 0).Union(cell(4, 0)), "Output", style), kCtrlTagOutput);
      pGraphics->AttachControl(new IVMeterControl<2>(cell(3, 1).Union(cell(4, 1)), "DC", style), kCtrlTagLim);

      pGraphics->AttachControl(new IVKnobControl(cell(3, 2).GetMidVPadded(buttonSize), maxTresh, "Max Thresh", style, false), kNoTag, "vcontrols");
      pGraphics->AttachControl(new IVSlideSwitchControl(cell(4, 2).GetMidVPadded(buttonSize), sendDiff, "Only Diff", style.WithShowValue(false).WithShowLabel(true).WithWidgetFrac(0.5f).WithDrawShadows(false), false), kNoTag, "vcontrols");



      // Curve box
      pGraphics->AttachControl(new IVPlotControl(cell(0, nCols - 5).Union(cell(2, nCols - 1)), {
                                                              {COLOR_DARK_GRAY , [](double x) { return getValueDisplay(x); } },
                                                              {COLOR_BLUE, [](double x) { return getStatR(x); } },
                                                              {COLOR_RED, [](double x) { return getStatL(x); } }


        }, 32, "IVPlotControl", style.WithShowLabel(false)), kNoTag, "vcontrols");
      // Sliders
      int gfromr = 2;

      pGraphics->AttachControl(new IVSliderControl(sliderCell(0, nCols - 1, 2), cp4, "4", style2));
      pGraphics->AttachControl(new IVSliderControl(sliderCell(0, nCols - 2, 2), cp3, "3", style2));
      pGraphics->AttachControl(new IVSliderControl(sliderCell(0, nCols - 3, 2), cp2, "2", style2));
      pGraphics->AttachControl(new IVSliderControl(sliderCell(0, nCols - 4, 2), cp1, "1", style2));
      pGraphics->AttachControl(new IVSliderControl(sliderCell(0, nCols - 5, 2), cp0, "0", style2));
  ...
  if (nChans == 2) { // Stereo Signal
    ...
    for (int s = 0; s < nFrames; s++) {
      ...
      for (int i = 0; i < nChans; i++) {
        ...
        if (s == 0) {
          y_buffer[i][id] = inputs[i][s] - last_input[i] + dcR * y_buffer[i][id_prev];
        }
        else {
          y_buffer[i][id] = inputs[i][s] - inputs[i][s - 1] + dcR * y_buffer[i][id_prev];
        }
...
sample IPlugEffect::getValue(const sample& s, const int &side) const
{
  double x, ret;
  if (s > 0.0) x = s / tresh;
  else x = -s / tresh;
...
void IPlugEffect::testPlug()
{
  ...
  const int size = PLUG_LATENCY/2-3, nloop=25;
...
void Limiter::ProcessBlock(sample** toProcess, const sample& thresh, const int& nFrames, const int& from, const int& buffSize)
{
  ...
  for (int s = 0; s < nFrames; ++s) {
    ...
    for (int i = 0; i < nChans; i++) {
      tmp = toProcess[i][id];
      if (toProcess[i][id] > thresh) {
        if (diff != NULL) diff[i][id] = (toProcess[i][id] - thresh) * 10.0;
        toProcess[i][id] = algo(toProcess[i][id], thresh);
      }
      else if (toProcess[i][id] < -thresh) {
        if (diff != NULL) diff[i][id] = (toProcess[i][id] + thresh) * 10.0;
        toProcess[i][id] = -algo(-toProcess[i][id], thresh);
      }
      else
        if (diff != NULL) diff[i][id] = 0.0;
    }
  }
...
sample Limiter::processSample(sample& smp, const sample& thresh)
{
  sample out = 0.0;
  if (smp > thresh) {
    if (diff != NULL) out = (smp - thresh) * 10.0;
    smp = algo(smp, thresh);
  }
...
Sigmoid::Sigmoid() : steep(0.0), c(1.0), v(1.0)
{
  setSteepness(1.0);
}
...
{
  if (steepness > 0.0 && steep != steepness) {
    steep = steepness;
